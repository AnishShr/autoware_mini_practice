...
class CameraTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        if len(local_path_msg.waypoints) > 0:
            ...
            waypoints = [(wp.pose.pose.position.x, wp.pose.pose.position.y) for wp in local_path_msg.waypoints]
            local_path = LineString(waypoints)
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        # Initialize empty lists to store the class, score and tfl_result for the traffic lights
        classes = []
        scores = []
        tfl_results = []

        # Assign empty list to store the rois in the camera image
        rois = []
        ...
        if len(stoplines_on_path) > 0:
            ...
            try:
                transform = self.tf_buffer.lookup_transform(target_frame=camera_image_msg.header.frame_id,
            ...
            # concatenate all rois in the current image with the empty rois list
        ...
        tfl_status.results = tfl_results
